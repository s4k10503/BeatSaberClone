...
namespace BeatSaberClone.Tests
{
    public class ScoreServiceTests
    {
        ...
        public void UpdateScore_WithNegativeMultiplier_ShouldThrowArgumentException()
        {
            ...
            // Act & Assert
            var ex = Assert.Throws<ArgumentException>(() => _scoreService.UpdateScore(_entityId, negativeMultiplier));
            Assert.That(ex.Message, Is.EqualTo("Multiplier must be greater than 0."));
        }
        ...
        public void UpdateCombo_WithNegativeMultiplier_ShouldThrowArgumentException()
        {
            ...
            // Act & Assert
            var ex = Assert.Throws<ArgumentException>(() => _scoreService.UpdateCombo(_entityId, negativeMultiplier));
            Assert.That(ex.Message, Is.EqualTo("Multiplier cannot be negative."));
        }
    }
}
